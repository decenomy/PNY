        ...
        stage("deploy_macos") {
            ...
            steps {
                ...
                sh """#!/bin/bash
                    ...
                    cp src/${BASE_NAME}d src/${BASE_NAME}-cli src/${BASE_NAME}-tx src/qt/${BASE_NAME}-qt ${BASE_NAME}.dmg deploy/macos/
                    cd deploy/macos
                    zip ${ZIP_NAME}-\$(git describe --abbrev=0 --tags | sed s/v//)-MacOS.zip ${BASE_NAME}d ${BASE_NAME}-cli ${BASE_NAME}-tx ${BASE_NAME}-qt ${BASE_NAME}.dmg
                    rm -f ${BASE_NAME}d ${BASE_NAME}-cli ${BASE_NAME}-tx ${BASE_NAME}-qt ${BASE_NAME}.dmg
                """
            }
        }
    }
