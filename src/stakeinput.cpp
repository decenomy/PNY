...
bool CPnyStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CPnyStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CPnyStake::GetTxOutFrom(CTxOut& out) const
{
...
CAmount CPnyStake::GetValue() const
{
...
CDataStream CPnyStake::GetUniqueness() const
{
...
bool CPnyStake::ContextCheck(int nHeight, uint32_t nTime)
{
