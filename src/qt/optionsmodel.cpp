...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::PNY);
    ...
    if (settings.contains("nAnonymizePnyAmount") || reset)
        SoftSetArg("-anonymizepnyamount", settings.value("nAnonymizePnyAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in pny.cpp during shotdown to also remove the window positions
