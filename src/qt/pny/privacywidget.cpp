...
#include "qt/pny/forms/ui_privacywidget.h"
#include "qt/pny/qtutils.h"
#include "guiutil.h"
#include "qt/pny/denomgenerationdialog.h"
...
PrivacyWidget::PrivacyWidget(PNYGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zPNY anonymizes your PNY by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zPNY or convert back to PNY"));
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zPNY");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zPNY");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zPNY");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zPNY");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zPNY");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zPNY");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zPNY");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zPNY");
    ...
    // Only Convert to PNY enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzPNY->setTitleClassAndText("btn-title-grey", "Total 0 zPNY");
    ui->btnTotalzPNY->setSubTitleClassAndText("text-subtitle", "Show denominations of zPNY owned.");
    ui->btnTotalzPNY->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select PNY outputs to mint into zPNY.");
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zPNY");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of PNY to mint into zPNY"));
    }else{
        btnText = tr("Convert back to PNY");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zPNY to convert back into PNY"));
    }
...
void PrivacyWidget::onTotalZpnyClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzPNY->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzPNY->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zPNY is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zPNY, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zPNY").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zPNY minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpny(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zPNY converted back to PNY"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    ui->btnTotalzPNY->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
