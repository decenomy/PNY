...
#include "qt/pny/forms/ui_send.h"
#include "qt/pny/addnewcontactdialog.h"
...
#include "qt/pny/myaddressrow.h"
#include "qt/pny/guitransactionsutils.h"
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-pny");
...
void SendWidget::refreshAmounts() {
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZpny ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendPny ? AskPassphraseDialog::Context::Send_PNY : AskPassphraseDialog::Context::Send_zPNY, true)){
...
bool SendWidget::sendZpny(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zPNY is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    if (walletModel->sendZpny(
        ...
        emit message("zPNY transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
