...
#include "qt/pny/forms/ui_dashboardwidget.h"
#include "qt/pny/sendconfirmdialog.h"
#include "qt/pny/txrow.h"
...
DashboardWidget::DashboardWidget(PNYGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of PNY staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquarePny, "square-chart-pny");
    setCssProperty(ui->labelSquarezPny, "square-chart-zpny");
    setCssProperty(ui->labelPny, "text-chart-pny");
    setCssProperty(ui->labelZpny, "text-chart-zpny");
    ...
    ui->labelAmountZpny->setText("0 zPNY");
    ui->labelAmountPny->setText("0 PNY");
    setCssProperty(ui->labelAmountPny, "text-stake-pny-disable");
    setCssProperty(ui->labelAmountZpny, "text-stake-zpny-disable");
    ...
    ui->btnHowTo->setText(tr("How to get PNY"));
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &PNYGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &PNYGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &PNYGUI::openFAQ);
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isPny = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZPNY;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            pny = (pair.first != 0) ? pair.first / 100000000 : 0;
            zpny = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalPny += pair.first;
            chartData->totalZpny += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesPny.append(pny);
        chartData->valueszPny.append(zpny);

        int max = std::max(pny, zpny);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    // init sets
    set0 = new QBarSet("PNY");
    set1 = new QBarSet("zPNY");
    set0->setColor(QColor(112,101,67));
    set1->setColor(QColor(207,200,175));
    ...
    set0->append(chartData->valuesPny);
    set1->append(chartData->valueszPny);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalPny > 0 || chartData->totalZpny > 0) {
        setCssProperty(ui->labelAmountPny, "text-stake-pny");
        setCssProperty(ui->labelAmountZpny, "text-stake-zpny");
    } else {
        setCssProperty(ui->labelAmountPny, "text-stake-pny-disable");
        setCssProperty(ui->labelAmountZpny, "text-stake-zpny-disable");
    }
    forceUpdateStyle({ui->labelAmountPny, ui->labelAmountZpny});
    ui->labelAmountPny->setText(GUIUtil::formatBalance(chartData->totalPny, nDisplayUnit));
    ui->labelAmountZpny->setText(GUIUtil::formatBalance(chartData->totalZpny, nDisplayUnit, true));
