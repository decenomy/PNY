...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl, bool fIncludeDelegations)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            Q_EMIT message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 PNY.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        Q_FOREACH (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal pny:URI (pny:XyZ...?message=example)
            {
...
bool WalletModel::createZpnySpend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZpny(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        CZerocoinSpendReceipt &receipt)
{
...
static void NotifyzPNYReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzPNYReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzPNYReset.connect(boost::bind(NotifyzPNYReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzPNYReset.disconnect(boost::bind(NotifyzPNYReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zpnyTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
