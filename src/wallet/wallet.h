...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zpnyspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 PNY at the same time
...
enum ZerocoinSpendStatus {
    ZPNY_SPEND_OKAY = 0,                            // No error
    ZPNY_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZPNY_WALLET_LOCKED = 2,                         // Wallet was locked
    ZPNY_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZPNY_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZPNY_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZPNY_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZPNY_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZPNY_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZPNY_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZPNY_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZPNY_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZPNY_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZPNY_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZPNY_SPENT_USED_ZPNY = 14,                      // Coin has already been spend
    ZPNY_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZPNY_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZPNYOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzpnyAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzPNYTracker> zpnyTracker;
    ...
    CzPNYWallet* getZWallet() { return zwalletMain; }
    ...
    /** zPNY reset */
    boost::signals2::signal<void()> NotifyzPNYReset;
