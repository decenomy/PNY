...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nDEPRECATED. Returns the current PNY address for receiving payments to this account.\n"
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    ...
    const bool fMintChange = params[1].get_bool();       // Mint change to zPNY
    ...
    if (Params().NetworkID() != CBaseChainParams::REGTEST) {
        if (fMintChange)
            throw JSONRPCError(RPC_WALLET_ERROR, "zPNY minting is DISABLED (except for regtest), cannot mint change");
...
UniValue spendzerocoinmints(const UniValue& params, bool fHelp)
{
    ...
    return DoZpnySpend(nAmount, false, true, vMintsSelected, address_str, isPublicSpend);
}
...
UniValue spendrawzerocoin(const UniValue& params, bool fHelp)
{
    ...
    return DoZpnySpend(mint.GetDenominationAsAmount(), false, true, vMintsSelected, address_str, isPublicSpend);
}
