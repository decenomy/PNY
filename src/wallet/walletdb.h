...
public:
    ...
    bool WriteZPNYSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZPNYSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZPNYSeed_deprecated(uint256& seed);
    bool EraseZPNYSeed();
    bool EraseZPNYSeed_deprecated();

    bool WriteZPNYCount(const uint32_t& nCount);
    bool ReadZPNYCount(uint32_t& nCount);
