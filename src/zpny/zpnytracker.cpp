...
CzPNYTracker::CzPNYTracker(std::string strWalletFile)
{
...
CzPNYTracker::~CzPNYTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzPNYTracker::Init()
{
...
bool CzPNYTracker::Archive(CMintMeta& meta)
{
...
bool CzPNYTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzPNYTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzPNYTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzPNYTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzPNYTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzPNYTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzPNYTracker::GetSerialHashes()
{
...
CAmount CzPNYTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzPNYTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzPNYTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzPNYTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzPNYTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzPNYTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzPNYTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzPNYTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzPNYTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzPNYTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzPNYTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzPNYWallet* zPNYWallet)
{
    bool iszPNYWalletInitialized = (NULL != zPNYWallet);
    CMintMeta meta;
    ...
    if (! iszPNYWalletInitialized)
        zPNYWallet = new CzPNYWallet(strWalletFile);
    meta.isSeedCorrect = zPNYWallet->CheckSeed(dMint);
    if (! iszPNYWalletInitialized)
        delete zPNYWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzPNYTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzPNYTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzPNYTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzPNYTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzPNYTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzPNYTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzPNYWallet* zPNYWallet = new CzPNYWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zPNYWallet);
        }
        delete zPNYWallet;
        LogPrint("zero", "%s: added %d dzpny from DB\n", __func__, listDeterministicDB.size());
    }
