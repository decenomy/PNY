...
CzPNYWallet::CzPNYWallet(std::string strWalletFile)
{
...
bool CzPNYWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZPNYCount(nCountLastUsed);
    else if (!walletdb.ReadZPNYCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzPNYWallet::Lock()
{
    seedMaster = 0;
}

void CzPNYWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzPNYWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZPNY(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzPNYWallet::LoadMintPoolFromDB()
{
...
void CzPNYWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzPNYWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzPNYWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zpnyTracker->HasPubcoinHash(pMint.first)) {
...
bool CzPNYWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZPNY(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    pwalletMain->zpnyTracker->Add(dMint, true);
...
void CzPNYWallet::SeedToZPNY(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzPNYWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzPNYWallet::GenerateDeterministicZPNY(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzPNYWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzPNYWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzPNYWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
