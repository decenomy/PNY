...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("%s : zPNY spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!txIn.IsZerocoinPublicSpend())
                    return state.Invalid(error("%s: failed for tx %s, every input must be a zcpublicspend",
                            __func__, tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zpny");

                libzerocoin::ZerocoinParams* params = consensus.Zerocoin_Params(false);
                PublicCoinSpend publicSpend(params);
                if (!ZPNYModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                    return false;
                }
                ...
                if (!CheckPublicCoinSpendVersion(publicSpend.getVersion())) {
                    return state.Invalid(error("%s : Public Zerocoin spend version %d not accepted. must be version %d.",
                            __func__, publicSpend.getVersion(), CurrentPublicCoinSpendVersion()), REJECT_INVALID, "bad-txns-invalid-zpny");
                }
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight <= 1) {
        ...
        nSubsidy = 10 * COIN;
    } else if (nHeight > 855000 && nHeight <= 900000) {
        nSubsidy = 8 * COIN;			
    } else if (nHeight > 900000 && nHeight <= 905000) {
        nSubsidy = 20 * COIN;			
    } else if (nHeight > 905000 && nHeight <= 950000) {
        nSubsidy = 8 * COIN;			
    } else if (nHeight > 950000 && nHeight <= 955000) {
        nSubsidy = 20 * COIN;			
    } else if (nHeight > 955000 && nHeight <= 1000000) {
        nSubsidy = 8 * COIN;			
    } else if (nHeight > 1000000 && nHeight <= 1005000) {
        nSubsidy = 20 * COIN;			
    } else if (nHeight > 1005000 && nHeight <= 1050000) {
        nSubsidy = 8 * COIN;			
    } else if (nHeight > 1050000 && nHeight <= 1055000) {
        nSubsidy = 20 * COIN;			
    } else if (nHeight > 1055000 && nHeight <= 1100000) {
        nSubsidy = 8 * COIN;			
    } else if (nHeight > 1100000 && nHeight <= 1105000) {
        nSubsidy = 20 * COIN;			
    } else if (nHeight > 1105000 && nHeight <= 1150000) {
        nSubsidy = 8 * COIN;			
    } else if (nHeight > 1150000 && nHeight <= 1155000) {
        nSubsidy = 20 * COIN;			
    } else if (nHeight > 1155000 && nHeight <= 1200000) {
        nSubsidy = 8 * COIN;			
    } else if (nHeight > 1200000 && nHeight <= 1205000) {
        nSubsidy = 20 * COIN;			
    } else if (nHeight > 1205000 && nHeight <= 1250000) {
        nSubsidy = 8 * COIN;			
    } else if (nHeight > 1250000 && nHeight <= 1255000) {
        nSubsidy = 20 * COIN;			
    } else if (nHeight > 1255000 && nHeight <= 1300000) {
        nSubsidy = 8 * COIN;			
    } else if (nHeight > 1300000 && nHeight <= 1305000) {
        nSubsidy = 20 * COIN;			
    } else if (nHeight > 1305000 && nHeight <= 1350000) {
        nSubsidy = 8 * COIN;			
    } else if (nHeight > 1350000 && nHeight <= 1355000) {
        nSubsidy = 20 * COIN;			
    } else if (nHeight > 1355000 && nHeight <= 1400000) {
        nSubsidy = 8 * COIN;			
    } else if (nHeight > 1400000 && nHeight <= 1405000) {
        nSubsidy = 20 * COIN;			
    } else if (nHeight > 1405000 && nHeight <= 2593500) {
        nSubsidy = 8 * COIN;
    } else if (nHeight > 2593500) {
        nSubsidy = 0 * COIN;
    }
    return nSubsidy;
}
...
CScript OpFundScriptPubKey()
{
    ...
    CBitcoinAddress address(devPayoutAddr);
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    if (!UpdateZPNYSupplyDisconnect(block, pindex))
        return error("%s: Failed to calculate new zPNY supply", __func__);
...
{
    util::ThreadRename("pny-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (!UpdateZPNYSupplyConnect(block, pindex, fJustCheck)) {
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN && pindex->nHeight == consensus.height_last_ZC_WrappedSerials + 1
        ...
        RecalculatePNYSupply(consensus.height_start_ZC, false);
    }
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (!Params().IsRegTestNet() && block.IsProofOfWork() && (pindexPrev->nHeight + 1 <= 0)) {
    ...
    if (block.nVersion >= 5 && block.nBits != nBitsRequired) {
        // Pny Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nPnyBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nPnyBadBlockBits)) {
            // accept PNY block minted with incorrect proof of work threshold
            return true;
        }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if((block.nVersion < 3 && nHeight >= consensus.height_start_BIP65) ||
        (block.nVersion < 4 && nHeight >= consensus.height_start_BIP65) ||
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if(hasZPNYInputs && in.IsZerocoinSpend())
            ...
            if(hasZPNYInputs) {
                for (const CTxIn& zPnyInput : zPNYInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zPnyInput);
