...
*Setup instructions for a gitian build of Peony Core using a VM or physical system.*
...
to the PNY GitHub release page.
...
- [Building Peony Core](#building-pny-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/pny-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/pny-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/pny-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/pny-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/pny-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/pny-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/PeonyTeam/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@pny.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/pny-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/pny-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/pny-osx-*-build.assert
```
