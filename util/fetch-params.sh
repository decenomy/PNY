...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/PNYParams"
    else
        PARAMS_DIR="$HOME/.pny-params"
    fi
fi
...
Failed to fetch the PNY zkSNARK parameters!
...
This script will fetch the PNY zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common PNY zkSNARK parameters. Note that it is
